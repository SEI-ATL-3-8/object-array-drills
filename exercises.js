...
album5.albumDetails.formats = album6.albumDetails.formats;
...
switch(true) {
  case numAlbums % 2 === 0 && numAlbums % 3 === 0:
    console.log(`The number ${numAlbums} is divisble by 2 or 3`);
    break;
  case numAlbums % 2 === 0:
...
let sireTally = 0;
for (let x in talkingHeadsAlbums) {
  if (talkingHeadsAlbums[x].albumDetails.label === "Sire" || (talkingHeadsAlbums[x].albumDetails.labels && talkingHeadsAlbums[x].albumDetails.labels.includes("Sire"))) sireTally +=1;
}
...
for (let x in tickets) {
  let message = `Welcome, ${tickets[x].name}.`;
  message += tickets[x].seats > 1 ? ' You and your party' : ' You';
  if (tickets[x].type === "premium") {
    message += ` may sit anywhere in the first 3 rows of the ${tickets[x].section} section.`
  }

  else {
    message += ` may sit anywhere except first 3 rows of the ${tickets[x].section} section.`
  }

  if (tickets[x].seats > 1) message += ' Please be sure to leave no seats between you.';
  console.log(message);
}
...
let ticketss = [
  ...
  {amount: 80.00, discount: false, zombie: false},
  {amount: 90.00},
  {amount: 50.00, discount: true}
];

for (let i in ticketss) {
  let message = "";
  let drinks = 0;
  
  if('amount' in ticketss[i] && 'discount' in ticketss[i] && 'zombie' in ticketss[i]) {
    let {amount,discount,zombie} = ticketss[i];

    if ( amount >= 90 || (amount >= 80 &&  discount)) {
      message = "PREMIER PLUS";
    }
   else {
     message += amount === 65 ? "PREMIER" : "STANDARD";
     if (discount) drinks += 10;
     if (zombie) drinks += 10;
     message += drinks ? ` $${drinks} DRINKS` : " NO DRINKS";
